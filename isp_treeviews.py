...
def renderPromptInvoices(parentWindow, invoiceList, gridRow, gridColumn):
  ...
  invoiceTable.grid(row=gridRow, column=gridColumn)
...
def renderPromptMulitTransactions(parentWindow, transactionList, gridRow, gridColumn):
  ...
  multiTransactionTable = ttk.Treeview(parentWindow, columns=('low_invoice', 'high_invoice', 'amount', 'paid_on', 'paid_by', 'payment_method', 'og_string'))
  ...
  multiTransactionTable.grid(row=gridRow, column=gridColumn)
